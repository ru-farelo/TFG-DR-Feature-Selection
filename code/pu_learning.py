...
import numpy as np
from code.data_processing2 import get_data_features
...
def select_reliable_negatives(
    ...
    if method == "similarity":
        ...
        # Ensure rn maintains the same structure as p for similarity method
        if p.ndim == 2 and rn.ndim == 1:
            rn = rn.reshape(-1, 1)
    ...
    # Ensure both p and rn have the same dimensionality
    if p.ndim == 2 and rn.ndim == 1:
        rn = rn.reshape(-1, 1)
    elif p.ndim == 1 and rn.ndim == 2:
        p = p.flatten()
        rn = rn.flatten()
